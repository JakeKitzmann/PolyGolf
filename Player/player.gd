...
func _ready():
	walking_camera.current = is_multiplayer_authority() # set walking camera as active
			...
			for club_type in clubs:
				...
				if club[0] == 1: # if the club is active
					if player.current_animation == club[1] and swing_once:  # swinging the club
						...
						shoot_ball(delta, shot_angle, deg_to_rad(club[3]), power)
...
func _input(event):
	...
	if event is InputEventKey and event.is_action_pressed("enter_shooting_mode") and at_ball:
		switch_cam()
